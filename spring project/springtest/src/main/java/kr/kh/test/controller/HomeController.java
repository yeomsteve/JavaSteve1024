...
public class HomeController {
	...
	public ModelAndView signupPost(ModelAndView mv, MemberVO member,
		...
		if(res) {
			...
			MessageUtils.alertAndMovePage(response, "회원가입 성공."
					contextPath, "/");
			mv.setViewName("redirect:/");
		}else {
			//실패했다고 알림 메세지(추후 구현 예정)
			MessageUtils.alertAndMovePage(response, "회원가입 실패.",
					contextPath, "/signup");
	...
	public ModelAndView logoutPost(ModelAndView mv, HttpSession session,
		...
		MessageUtils.alertAndMovePage(response, "로그아웃 성공.",
				contextPath, "/");
		mv.setViewName("redirect:/");
		return mv;
	}
	...
	public ModelAndView emailAuthentication(ModelAndView mv, MemberOKVO mok,
		...
		if(res) {
			//인증 성공 메세지
			MessageUtils.alertAndMovePage(response, "이메일 인증 성공.",
					contextPath, "/");
		}else {
			//인증 실패 메세지
			MessageUtils.alertAndMovePage(response, "이메일 인증 실패.",
					contextPath, "/");
		}
		mv.setViewName("redirect:/");
		return mv;
