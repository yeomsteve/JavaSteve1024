...
join
- 두개 이상의 테이블을 합쳐서 검색
- 이유 : 알고있는 데이터와 조회하는 데이터가 서로 다른 테이블에 있고, 테이블들을 속성으로 연결할수있는경우 사용
inner join
 - 두 테이블을 연결시 연결되는 속성이 있는 값들만 연결
 outer join
 -두 테이블을 연결시 연결되는 속성이 없는 값들도 연결
-- 테이블1과 테이블2가 연결
...
*/
-- 제품명과 옵션명을 함께 조회
select pr_title, po_title, po_amount from product join product_option on po_pr_num = pr_num;

-- abc 회원이 구매한 제품옵션번호를 조회하는 쿼리. 단, 중복된 제품 옵션이 있으면 제거
