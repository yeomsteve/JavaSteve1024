...
- dense_rank():
	-같은 값이 있을때 등수가 같음
	-같은 값이 있는 행 다음 등수는 같은값이 있는 등수 +1
- rank():
	-같은 값이 있을때 등수가 같음
...
select 
    ...
    select dense_rank() over(order by pr_price desc) as 가격순위,
	pr_title as 제품명,
	pr_price as 제품가격 
    from product;
    
    select	row_number() over(order by pr_price desc) as 가격순위,
	pr_title as 제품명,
	pr_price as 제품가격 
    from product;
    
    select ntile(3) over(order by pr_price desc) as 가격순위,
	pr_title as 제품명,
	pr_price as 제품가격 
    from product;
