/*
-- 트리거에서 추가/수정/삭제된 데이터를 가져올때 NEW | OLD를 사용
-- INSERT: NEW(추가된 데이터)
	-- INSERT된 데이터는 트리거에서 수정할수 없음
-- UPDATE: OLD(값이 변경전 데이터), NEW(값이 변경후 데이터)
-- DELETE : OLD(삭제된 데이터)
...
DELIMITER //
CREATE TRIGGER 트리거명
-- BEFORE : 테이블에 데이터가 삭제|추가|수정 되기전에 트리거가 실행
...
VALUES('QWE2022122111712', 1, '서울시 강남', '12345', 'qwe', 2);